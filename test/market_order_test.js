...
contract.skip("Dex", accounts => {
    //When creating a market sell order, the seller must have enough tokens for the trade
    it("should throw an error if token balance is to low", async () => {
    ...
    it("Market orders should not fill more limit orders than the order amount", async () => {
        ...
        assert(orderbook.length == 0, "Sell side Orderbook should be empty ar start of test");
        ...
        //Deposit LINK into DEX accounts 1, 2, 3
        ...
        //create market order that shoul fill 2/3 orders in the book
        ...
        assert(orderbook[0].filled, "Sell side order should have 0 filled");
    ...
    it("Market orders should be filled until the orderbook is empty", async () => {
        ...
        //Fill up the sell orderbook again
    ...
    it("The eth balance of the buyer should decrease with the filled amount", async () => {
        ...
        //seller deposit link and creates a sell limit order for 1 link for 300 wei
    ...
    it("Should throw an error when creating a buy order without adequate ETH balance", async () => {
        let dex = await Dex.deployed()
